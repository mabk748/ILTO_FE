...
function targetLabel(target: LearningTarget): string {
...
function Editor({
  ...
  const mutation = useMutation({
    mutationFn: async () =>
      mode === "delete"
        ...
        : saveLearningResource(target, values),
    retry: false,
  });
  ...
  const field = (
    ...
    type = "text",
    required = false,
    extra?: { maxLength?: number; min?: string },
  ) => (
  ...
  const kind = target.kind === "roadmap" ? "roadmap" : "reading entry";
